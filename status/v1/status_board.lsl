...
list buildStatusLines(string name, integer ageDays, integer gender, integer pregval, integer fervor)
{
    ...
    integer nameLen = 25;
    //string safeName = llGetSubString(name, 0, nameLen - 1); 
    string safeName = truncateName(name, nameLen);
    ...
    if (pregLabel != "--")
    {
        line2 = "Preg " + pregLabel + " | Ferv " + fervorLabel;
    }
    else {
        line2 = "Ferv " + fervorLabel;
    }
...
updateHoverText()
{
    ...
    while (i < count)
    {
        ...
        if (llGetListLength(parsed) > 0)
        {
            ...
            if (llGetListLength(lines) >= 2)
            {
                ...
                combinedLines += [nameLine, statsLine, dividerLine];
            }
    ...
    else
    {
        ...
        string finalText = "Horses: " + (string)count + "\n" + "---" + "\n" + combined;
        llSetText(finalText, llList2Vector(gTextColors, gNameColorIndex), 1.0);
    }
}
