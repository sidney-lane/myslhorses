...
sayStats(key agent)
{
    ...
    llRegionSayTo(agent, 0, "Text Color 1: " + nameColorLabel);
    llRegionSayTo(agent, 0, "Text Color 2: " + statusColorLabel);
...
handleMenuSelection(key agent, string message)
{
    ...
    else if (message == "TEXT COLOR 1")
    {
        if (agent == llGetOwner())
        {
            showColorMenu(agent, "1");
        }
    }
    else if (message == "TEXT COLOR 2")
    {
        if (agent == llGetOwner())
        {
            showColorMenu(agent, "2");
        }
    }
